...
public class WifidogController extends BaseController {
	...
	public void login() {
		...
		if (wxConfig != null) {
			if (StringUtils
					.startsWith(url.trim(), getProperty().getWeixinUrl())) {
	...
	public void loginValidate() {
		...
		WifidogLog log = new WifidogLog();
		if (param == null) {
			logger.error("loginValidate接口会话(Session)过期了");
			setAttr("title", "出错误了~");
			setAttr("errorMsg", "会话过期了，请联系管理员！");
			render("/error/error.html");
			return;
		}
		// int busId = acInfo.getInt("businessid");
		// //通过mac地址设置黑名单
		// BwList bwlist = BwList.dao.selectBwListByMac(busId,
		// Constants.WHTYPE_BLACK, param.getMac());
		// if(bwlist != null){
	...
	public void loginDownload(){
		...
		// 认证类型
		setAttr("authtype", Constants.AUTHTYPE_ONKEY);		//一键上网
		setAttr("param", param);
		setAttr("busId", businessid);
		
		// 认证页模板切换
		BusinessTemplet busTemplet = BusinessTemplet.dao
						.selectBusTemplate(businessid);
		...
		BusinessInfo busInfo = BusinessInfo.dao.selectBusinessInfo(businessid);
		String apkurl = null;

		if (busInfo != null) {
			String fileProxy = getProperty().getFileProxyDomain();
			apkurl = fileProxy + busInfo.getStr("apkurl");
		}
		setAttr("businessName", busInfo.get("busname"));
		...
		appLog.setAcId(acId);
		appLog.setBusinessId(businessid);
		// 手机助手AppId默认为0
		appLog.setAppId(0);
		appLog.setVersion("1.0");
		// 写入手机助手下载日志
		this.writeAppDownloadLog(appLog);

		setAttr("apkurl", apkurl);
		
		render("/auth/login_tmpl6.html");
	}
	...
	public void auth() {
		...
		if (stage.equals("login")) {
			...
			try {
				MemCachedUtil.replace("ClientInfo_" + mac, clienttimeout * 60,
						clientInfo);
			} catch (Exception e) {
	...
	public void portal() {
		...
		if (param != null) {
			...
			logger.error("Portal接口 会话(Session)过期了");
			log.setBusinessId(String.valueOf(businessid));
		}
		...
		if (acAuth == null) {
			...
			if (Constants.AFTERAUTH_PORTAL == acAuth.getInt("afterauth")) {
				redirectUrl = getProperty().getDefaultPortalUrl();
					...
					|| Constants.AFTERAUTH_APK == acAuth.getInt("afterauth")) {
		...
		if (StringUtils.contains(redirectUrl, getProperty()
			...
			if (param != null) {
				sb.append("&mac=").append(param.getMac());
			}
		...
		int authid = busTemplet.getInt("authid");
//		if(authid == Constants.DOWNLOADAPP){
//			
//			AppDownLoadLog appLog = new AppDownLoadLog();
//			appLog.setActionTime(DateFormatUtils.format(time,
//					Constants.DATETIME_FORMAT));
...
//
//			setAttr("apkurl", apkurl);
//			setAttr("portalUrl", redirectUrl);
//			// 调转到路由器Web 应用套餐下载页面
//			// setAttr("portalUrl", "http://192.168.6.1:9090");
